    ...
    public static UndergroundLayer getPlayerUndergroundLayer(Player player) {
        ...
        for (var layer : Config.undergroundLayers) {
            for (var regionName : playerRegions) {
                if (layer.getWorldGuardRegions().stream().anyMatch(i -> i.equalsIgnoreCase(regionName))) {
                    if (bestLayer == null) {
                        bestLayer = layer;
                    } else {
                        if (layer.getPriority() > bestLayer.getPriority()) {
                            bestLayer = layer;
                        } else if (layer.getPriority() == bestLayer.getPriority() && 
                                   layer.getRenderFromY() < bestLayer.getRenderFromY()) {
                            bestLayer = layer;
                        }
                    }
                    break;
